...
class guiObj4(guiBase):
    ...
    def exeButton(self,event,msgTxt):
        self.mbox1 = lv.msgbox(lv.screen_active(), "GPL License", msgTxt , self.btns, True)
    ...
    def findDef(self,event,hfn):
        ...
        if sys.platform == 'linux':
            IMGPATH = "/home/angel/Documentos/Github/galdeano-lv/help/"
        else:
            IMGPATH = "/help/"
        hfn = IMGPATH+self.ta.get_text()
        hfn = hfn.replace('(','')
        hfn = hfn.replace(')','')
        hfn = hfn + ".help"
        try:
