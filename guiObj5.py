...
class guiObj5(guiBase):
    ...
    def DeleteItem(self,e,localizacion,cibusTabula):
        obj = e.get_target()
        #obtenemos la etiq
        label=obj.get_child(0)
        comunication.cibusTabulaConsume(label.get_text())
        self.llenaCibusTabula(localizacion,cibusTabula)

    def change_event_cb(self,e,ta,cibusTabula):
        obj = e.get_target()
        row = lv.C_Pointer()
        col = lv.C_Pointer()
    ...
    def ct_location_menu(self,e,cibusTabula):
        dropdown = e.get_target()
        print(dropdown.get_selected())
        print(self.ct_locations[dropdown.get_selected()])
        self.ct_selected_location=self.ct_locations[dropdown.get_selected()]
        self.llenaCibusTabula( self.ct_selected_location ,cibusTabula)
        

    #boton exe
    def tabula_exe(self,e,ta,cibusTabula):
        datos = comunication.cibusTabulaAlta(ta.get_text(),self.ct_selected_location)
        self.llenaCibusTabula(self.ct_selected_location,cibusTabula)
        
    def llenaCibusTabula(self,localizacion,cibusTabula):
        ...
        for idx, product in enumerate(datos):
            ...
            btn1.add_event_cb(lambda e:self.DeleteItem(e,localizacion,cibusTabula), lv.EVENT.CLICKED, None)
    ...
    def execScreen(self):
        ...
        miTeclado.execFunc = lambda e: self.tabula_exe(e,ta,cibusTabula)
        
        dropdown = lv.dropdown(lv.scr_act())
        dropdown.align(lv.ALIGN.TOP_LEFT, 165, 25)
        dropdown.set_options("\n".join(self.ct_locations))
        dropdown.add_event_cb(lambda e: self.ct_location_menu(e, cibusTabula) , lv.EVENT.VALUE_CHANGED, None)
        ...
        btn1.add_event_cb(lambda e:self.tabula_exe(e,ta,cibusTabula), lv.EVENT.CLICKED, None)
    ...
    def execScreenConf(self):
        ...
        label.set_text("execScreenConf CibusTabula")
        label.center()
